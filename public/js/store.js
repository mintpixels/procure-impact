...
var Brand = /*#__PURE__*/function () {
  ...
  _createClass(Brand, [{
      ...
      this.vm = Vue.createApp({
        ...
        methods: {
          getImagePath: function getImagePath(path) {
            return "https://images.takeshape.io/".concat(path);
          },
          getButtonTarget: function getButtonTarget(button) {
            if (!button) return '';
            return button.openNewWindow ? '_blank' : '';
          },
          getButtonUrl: function getButtonUrl(button) {
            if (!button) return '';
            return button.url;
          },
          getButtonText: function getButtonText(button) {
            if (!button) return '';
            return button.text;
          }
        }
    ...
    key: "loadBrand",
    value: function loadBrand() {
      var vm = this.vm;
      axios.get("/data/brands/".concat(this.handle)).then(function (response) {
...
var Cart = /*#__PURE__*/function () {
  ...
  _createClass(Cart, [{
      ...
      if ($('#side-cart').length > 0) {
        ...
        this.vm = Vue.createApp({
          ...
          methods: {
            ...
            hideSideCart: function hideSideCart() {
              root.hideSideCart();
            }
          }
        }).mount('#side-cart'); // Load cart data.
    ...
    key: "addVariants",
    value: function addVariants(variants, errorHandler) {
      this.vm.error = '';
    ...
    key: "addItemWithOptions",
    value: function addItemWithOptions(id, quantity, options, errorHandler) {
      this.vm.error = '';
      axios.post('/cart/items', {
        id: id,
        quantity: quantity,
    ...
    key: "addItem",
    value: function addItem(id, quantity, errorHandler) {
      this.addItemWithOptions(id, quantity, [], errorHandler);
    }
  }, {
    key: "updateItem",
    value: function updateItem(id, quantity, errorHandler) {
      var self = this;
      this.vm.error = '';
    ...
    key: "removeItem",
    value: function removeItem(id, errorHandler) {
      var self = this;
      this.vm.error = '';
    ...
    key: "goToCheckout",
    value: function goToCheckout(errorHandler) {
      axios.post('/cart/checkout').then(function (response) {
    ...
    key: "setDealer",
    value: function setDealer(dealer) {
      var self = this;
      axios.post('/cart/dealer', {
        id: dealer.id
      }).then(function (response) {
        self.vm.cart = response.data.cart;
        self.scrollToTop();
...
var Checkout = /*#__PURE__*/function () {
  ...
  _createClass(Checkout, [{
    ...
    value: function init() {
      ...
      if ($('#checkout').length > 0) {
        ...
        this.vm = Vue.createApp({
          ...
          methods: (_methods = {
            validShippingAddress: function validShippingAddress(shipment) {
              if (this.shipToDealer(shipment)) return true;
              if (this.checkout.use_billing) return true;
              return shipment.address.first_name && shipment.address.last_name && shipment.address.address1 && shipment.address.city && shipment.address.state && shipment.address.zip && shipment.address.phone;
            },
            selectBillingAddress: function selectBillingAddress() {
              var address = ctx.lookupAddress(this.checkout.billing.id);
              this.checkout.billing = ctx.getAddressParams(address);
            },
            selectShippingAddress: function selectShippingAddress(index) {
              var shipment = this.checkout.shipments[index];
              var address = ctx.lookupAddress(shipment.address.id);
              shipment.address = ctx.getAddressParams(address);
              ctx.addressChanged(index);
            },
            shipToDealer: function shipToDealer(shipment) {
              return shipment.ffl_required && this.checkout.dealer;
            },
            removeDealer: function removeDealer() {},
            updateInsurance: function updateInsurance() {
            ...
            validCheckout: function validCheckout() {
              return this.termsAccepted && (this.houseAccount || this.checkout.is_pickup || this.validCard());
            },
            addressChanged: function addressChanged(shipment) {
            ...
            saveUseBilling: function saveUseBilling() {
              ctx.saveUseBilling();
            },
            saveBilling: function saveBilling(billing) {
            ...
            methodChanged: function methodChanged(shipment, index) {
              ctx.saveMethod(shipment, index);
            },
            shippingCanBeDifferent: function shippingCanBeDifferent() {
              return true;
          ...
          }, _defineProperty(_methods, "removeDealer", function removeDealer() {
            ctx.removeDealer();
          }), _defineProperty(_methods, "deliveryDate", function deliveryDate(days, carrier) {
            if (!days) return '';
            var d = new Date();
            var day = d.getDay(); // Add padding for shipping delay. With extra
            // delay around the weekend.

            var pad = 1;
            if (day == 5) pad = 3;else if (day == 6) pad = 2;
            days += pad;
            var newDay = (day + days) % 7; // USPS doesn't deliver on Sunday.

            if (carrier == 'USPS' && newDay == 0) {
              days++;
            } // UPS doesn't deliver on Saturday or Sunday.
            else if (carrier == 'UPS') {
              if (newDay == 6) days += 2;else if (newDay == 0) days += 1;
            }
    ...
    value: function validAddress(address) {
      return address && address.first_name && address.last_name && address.address1 && address.city && address.state && address.zip && address.phone;
    }
  }, {
    key: "lookupAddress",
    value: function lookupAddress(id) {
      return this.vm.checkout.addresses.filter(function (address) {
        return address.id == id;
      })[0];
    }
  }, {
    key: "validateAddresses",
    value: function validateAddresses() {
      var shipments = this.vm.checkout.shipments;

      for (var i = 0; i < shipments.length; i++) {
        var address = shipments[i].address;
        this.setAddressKey(address);
        shipments[i].addressValid = address.first_name && address.last_name && address.address1 && address.city && address.state && address.zip && address.phone ? true : false;
      }
    }
  }, {
    key: "addressChanged",
    value: function addressChanged(index) {
      // Since an address changed, we need to validate the 
      // address and check if shipping methods need to be
      // recalculated.
      var address = this.vm.checkout.shipments[index].address;
      this.setAddressKey(address);
      this.vm.checkout.shipments[index].addressValid = address.first_name && address.last_name && address.address1 && address.city && address.state && address.zip && address.phone ? true : false;
      if (this.debounceTimer) return; // Check if we need to reload shipping methods.
    ...
    key: "validShipping",
    value: function validShipping() {
      if (!this.vm) return;
      return true;
    }
  }, {
    key: "reloadAllMethods",
    value: function reloadAllMethods(indexes) {
      return;
      var shipments = [];

      for (var i = 0; i < indexes.length; i++) {
        var index = indexes[i];
        var shipment = this.vm.checkout.shipments[index];
        ...
        if (this.vm.validShippingAddress(shipment) && (this.vm.shipToDealer(shipment) || shipment.addressKey != shipment.address.key)) {
          var address = this.getAddressParams(shipment.address);
      ...
      if (shipments.length > 0) {
        ...
        var ctx = this;
        axios.post("/data/checkout/".concat(this.guid, "/shipments/methods"), params).then(function (response) {
    ...
    key: "getAddressParams",
    value: function getAddressParams(address, company) {
      return {
        id: address.id,
    ...
    key: "setAddressKey",
    value: function setAddressKey(address) {
      address.key = this.getAddressKey(address);
    }
  }, {
    key: "getAddressKey",
    value: function getAddressKey(address) {
    ...
    value: function get() {
      var ctx = this;
      axios.get("/data/checkout/".concat(this.guid)).then(function (response) {
        ctx.setCheckout(response.data.checkout);
    ...
    value: function signOut() {
      var ctx = this;
      axios.post("/checkout/".concat(this.guid, "/signout")).then(function (response) {
        ctx.setCheckout(response.data.checkout);
        ctx.vm.stepIndex = 1;
    ...
    key: "attemptSignin",
    value: function attemptSignin(email, password) {
      var params = {
        email: email,
        password: password
      };
      ...
      var ctx = this;
      vm.signinError = '';
      axios.post("/checkout/".concat(this.guid, "/signin"), params).then(function (response) {
    ...
    value: function getAuthToken() {
      ...
      var cardData = {};
      var exp = this.vm.card.expiry.split('/');
      ...
      Accept.dispatchData(secureData, function (response) {
        if (response.messages.resultCode == 'Ok') {
          _this.vm.card.token = response.opaqueData.dataValue;

          _this.completeCheckout();
        } else {
          _this.vm.paymentError = response.messages.message[0].text;
        }
      });
    }
  }, {
    key: "completeCheckout",
    value: function completeCheckout() {
      ...
      vm.submitting = true;
      vm.errorProducts = [];
      axios.post("/checkout/".concat(this.guid, "/complete"), params).then(function (response) {
        window.location.reload();
      })["catch"](function (error) {
        vm.errorMessage = error.response.data.error;
        vm.submitting = false;
        if (error.response.data.products) vm.errorProducts = error.response.data.products;
      });
    }
  }, {
    key: "saveCustomer",
    value: function saveCustomer() {
      ...
      var self = this;
      axios.post("/checkout/".concat(this.guid, "/customer"), params).then(function (response) {
        self.setCheckout(response.data.checkout);
      });
    }
  }, {
    key: "savePickup",
    value: function savePickup() {
      var params = {
        is_pickup: this.vm.checkout.is_pickup
      };
      var self = this;
      axios.post("/checkout/".concat(this.guid, "/pickup"), params).then(function (response) {
        self.setCheckout(response.data.checkout);
      });
    }
  }, {
    key: "saveUseBilling",
    value: function saveUseBilling() {
      var params = {
        use_billing: this.vm.checkout.use_billing
      };
      var ctx = this;
      axios.post("/checkout/".concat(this.guid, "/usebilling"), params).then(function (response) {
        ctx.setCheckout(response.data.checkout);
        var shipments = [];

        for (var i = 0; i < ctx.vm.checkout.shipments.length; i++) {
          if (!(ctx.vm.checkout.shipments[i].ffl_required && ctx.vm.checkout.dealer)) shipments.push(i);
        }
    ...
    key: "saveInsurance",
    value: function saveInsurance() {
      var params = {
        accepts_insurance: this.vm.checkout.accepts_insurance
      };
      var self = this;
      axios.post("/checkout/".concat(this.guid, "/insurance"), params).then(function (response) {
        self.setCheckout(response.data.checkout);
      });
    }
  }, {
    ...
    value: function saveTax() {
      var ctx = this;
      axios.post("/checkout/".concat(this.guid, "/tax")).then(function (response) {
        ctx.setCheckout(response.data.checkout);
      });
    }
  }, {
    ...
    value: function setBillingAddress(billing) {
      var ctx = this;
      var params = this.getAddressParams(billing);
      axios.post("/checkout/".concat(this.guid, "/billing"), params).then(function (response) {
        ctx.setCheckout(response.data.checkout);
        var checkout = response.data.checkout;
        var shipments = [];

        for (var i = 0; i < checkout.shipments.length; i++) {
          if (checkout.use_billing || checkout.shipments[i].ffl_required && checkout.dealer) shipments.push(i);
        }
    ...
    value: function setCheckout(checkout) {
      ...
      for (var i = 0; i < this.vm.checkout.shipments.length; i++) {
        if (!this.vm.checkout.shipments[i].address.id) this.vm.checkout.shipments[i].address.id = '';
      }

      if (checkout.accepts_insurance) this.vm.insuranceChosen = true;
      if (!this.vm.insuranceChosen) checkout.accepts_insurance = '';
    }
  }, {
    key: "saveMethod",
    value: function saveMethod(shipment, index) {
      var params = {
        methodId: shipment.methodId
      };
      var self = this;
      axios.post("/data/checkout/".concat(this.guid, "/shipments/").concat(index, "/method"), params).then(function (response) {
        self.setCheckout(response.data.checkout);
      });
    }
  }, {
    key: "setShippingAddress",
    value: function setShippingAddress() {}
  }, {
    key: "addInsurance",
    value: function addInsurance() {}
  }, {
    key: "removeInsurance",
    value: function removeInsurance() {}
  }, {
    key: "addDealer",
    value: function addDealer() {}
  }, {
    key: "removeDealer",
    value: function removeDealer() {
      var ctx = this;
      axios.post("/checkout/".concat(this.guid, "/removedealer")).then(function (response) {
        ctx.setCheckout(response.data.checkout);
        var shipments = [];

        for (var i = 0; i < ctx.vm.checkout.shipments.length; i++) {
          if (ctx.vm.checkout.shipments[i].ffl_required) shipments.push(i);
        }
...
var Common = /*#__PURE__*/function () {
  ...
  _createClass(Common, [{
    ...
    value: function initMenu() {
      var root = this; // if($('#menu').length > 0) {
    ...
    key: "bindEvents",
    value: function bindEvents() {
      var root = this; // Hide overlay and any overlaid items.
...
var PDP = /*#__PURE__*/function () {
  ...
  _createClass(PDP, [{
    ...
    value: function init() {
      ...
      this.getProduct();
      this.getReviews();
      this.bindEvents(); // Check if we've come here from a review email.
      ...
      if (window.location.hash.indexOf('email') > 0) {
        ...
        setTimeout(function () {
          ...
          for (var i = 0; i < fields.length; i++) {
            if (fields[i].indexOf('email') >= 0) ctx.vm.addReview.email = fields[i].substr(6).replace('%40', '@');else if (fields[i].indexOf('name') >= 0) ctx.vm.addReview.name = fields[i].substr(5);
          }
      ...
      this.vm = Vue.createApp({
        ...
        data: function data() {
          ...
          return _ref = {
            ...
            imagesFilter: '',
            searchFilter: ''
          }, _defineProperty(_ref, "questions", []), _defineProperty(_ref, "showReviews", true), _defineProperty(_ref, "showQuestions", false), _defineProperty(_ref, "addingReview", false), _defineProperty(_ref, "addReview", {
            name: '',
            email: '',
            title: '',
            content: '',
            score: ''
          }), _defineProperty(_ref, "reviewSubmitted", false), _defineProperty(_ref, "submitError", false), _defineProperty(_ref, "addingQuestion", false), _defineProperty(_ref, "addQuestion", {
            name: '',
            email: '',
            question: ''
          }), _defineProperty(_ref, "questionSubmitted", false), _defineProperty(_ref, "modalView", ''), _defineProperty(_ref, "wishlists", []), _defineProperty(_ref, "wishlistName", ''), _defineProperty(_ref, "wishlistId", ''), _defineProperty(_ref, "wishlistMessage", ''), _defineProperty(_ref, "wishlistAvailable", false), _ref;
        },
        methods: {
          setActiveImage: function setActiveImage(image, i) {
          ...
          youtubeThumbnail: function youtubeThumbnail(code) {
            return "https://img.youtube.com/vi/".concat(code, "/hqdefault.jpg");
          },
          youtubeUrl: function youtubeUrl(code) {
            return "https://www.youtube.com/embed/".concat(code);
          },
          checkShip: function checkShip(zip) {
            root.canShip(zip, root.id);
          },
          formatMoney: function formatMoney(price) {
            return Util.formatMoney(price);
          },
          getFullStars: function getFullStars(avg) {
            return Math.floor(avg);
          },
          getHalfStars: function getHalfStars(avg) {
            var diff = parseFloat(avg) - this.getFullStars(avg);
            if (diff > .25 && diff < .9) return 1;
            return 0;
          },
          getEmptyStars: function getEmptyStars(avg) {
          ...
          qtyChanged: function qtyChanged() {
            ctx.setQuantityPrice();
          },
          viewImages: function viewImages(review, index) {
          ...
          submitReview: function submitReview() {
            if (this.addReview.name && this.addReview.email && this.addReview.content && this.addReview.score) {
              root.addReview(ctx.id, this.addReview.name, this.addReview.email, this.addReview.title, this.addReview.content, this.addReview.score); // Reset the form.
          ...
          submitQuestion: function submitQuestion() {
            if (this.addQuestion.name && this.addQuestion.email && this.addQuestion.question) {
              root.addQuestion(this.productId, this.addQuestion.name, this.addQuestion.email, this.addQuestion.question); // Reset the form.
          ...
          getShortDesc: function getShortDesc(desc) {
            // return desc;
            if (!desc) return '';
          ...
          showAddToWishlist: function showAddToWishlist(product) {
    ...
    key: "setQuantityPrice",
    value: function setQuantityPrice() {
    ...
    key: "addReview",
    value: function addReview(productId, name, email, title, content, rating) {
      var params = {
      ...
      $.post("/products/".concat(this.id, "/reviews"), params, function (response) {});
    }
  }, {
    key: "getReviews",
    value: function getReviews(page, sortBy, rating, images, search, callback) {
      var params = {
        product_id: this.id,
        page: page ? page : 0,
      ...
      var self = this;
      axios.get("/data/products/".concat(this.id, "/reviews"), {
        params: params
      }).then(function (response) {
        ...
        if (response.data.total % 5 == 0) self.vm.pageCount--;
    ...
    key: "canShip",
    value: function canShip(zip, product) {
      this.vm.shipMessage = '';
      this.vm.shipRules = [];
    ...
    key: "notifyMe",
    value: function notifyMe(email, product, sku, callback) {
      $.post('/api/notify', {
        email: email,
        product: product,
        sku: sku
...
var Search = /*#__PURE__*/function () {
  ...
  _createClass(Search, [{
    ...
    value: function initSearchPage() {
      ...
      this.vm = Vue.createApp({
        ...
        methods: {
          ...
          toggleShowAll: function toggleShowAll(item) {
            item.showAll = !item.showAll;
            this.$forceUpdate();
          },
          toggleFacet: function toggleFacet(facet) {
          ...
          toggleOption: function toggleOption(option) {
            ...
            if (option.chosen) this.chosenOptions.push(option.name);else this.chosenOptions.remove(option.name);
            this.$forceUpdate();
            root.search(true);
          },
          ...
          changeCategory: function changeCategory(option) {
            this.page = 1;
            this.categoryId = option ? option.id : '';
            this.$forceUpdate();
            root.search(true);
          },
          removeOption: function removeOption(name) {
            this.page = 1;
            this.chosenOptions.remove(name);

            for (var i = 0; i < this.facets.length; i++) {
              for (var j = 0; j < this.facets[i].options.length; j++) {
                if (this.facets[i].options[j].name == name) this.facets[i].options[j].chosen = false;
              }
            }
    ...
    value: function search(skip) {
      ...
      filter = filters.join('@');
      var g = '';
      if ($('[data-customer-group]').length > 0) g = $('[data-customer-group]').attr('data-customer-group');
      ...
      if (this.vm.mode == 'search') window.history.pushState({}, '', q);
      var self = this;
...
var Util = /*#__PURE__*/function () {
  ...
  _createClass(Util, [{
    ...
    value: function getProperty(name) {
      return $("[".concat(name, "]")).attr(name);
    }
  }, {
    key: "formatMoney",
    value: function formatMoney(p) {
      console.log('fm', p);
    ...
    key: "formatDateTime",
    value: function formatDateTime(d) {
      var day = dayjs(d);
      var now = dayjs();
      if (day.format('YYYY-MM-DD') == now.format('YYYY-MM-DD')) return day.format('[Today at] h:mma');
    ...
    key: "getCustomerName",
    value: function getCustomerName(c) {
      if (c) return "".concat(c.first_name, " ").concat(c.last_name);
      return '';
    }
  }, {
    key: "getCustomerLink",
    value: function getCustomerLink(c) {
      if (c) return "/admin/customers/".concat(c.id);
      return '';
    }
  }, {
    key: "showAsyncStatus",
    value: function showAsyncStatus(status, error) {
      var $elem = $('#async-status');
    ...
    key: "checkChanged",
    value: function checkChanged(a, b, ignore) {
      if (a == null) a = '';
      if (b == null) b = '';
      if (ignore == null) ignore = [];

      if (Array.isArray(a)) {
        if (a.length != b.length) return true; // We have to do a recursive check on the array elements.
        ...
        for (var k in a) {
          ...
          if (this.checkChanged(a[k], b[k], ignore)) return true;
        }

        for (var _k in b) {
          if (ignore.indexOf(_k) >= 0) continue;
          if (this.checkChanged(a[_k], b[_k], ignore)) return true;
        }
      } else if (a != b) {
        return true;
      }
...
__webpack_require__(/*! ./common */ "./resources/js/store/common.js");

__webpack_require__(/*! ./home */ "./resources/js/store/home.js");

__webpack_require__(/*! ./search */ "./resources/js/store/search.js");

__webpack_require__(/*! ./pdp */ "./resources/js/store/pdp.js");

__webpack_require__(/*! ./brand */ "./resources/js/store/brand.js");

__webpack_require__(/*! ./cart */ "./resources/js/store/cart.js");

__webpack_require__(/*! ./checkout */ "./resources/js/store/checkout.js");
})();
